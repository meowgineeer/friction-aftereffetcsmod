...
BasicTransformAnimator::BasicTransformAnimator() :
    ...
    mZDepthAnimator = enve::make_shared<QrealAnimator>("z_depth");
    mZDepthAnimator->setCurrentBaseValue(0);

    mXRotAnimator = enve::make_shared<QrealAnimator>("x_rotation");
    mXRotAnimator->setCurrentBaseValue(0);

    mYRotAnimator = enve::make_shared<QrealAnimator>("y_rotation");
    mYRotAnimator->setCurrentBaseValue(0);
    ...
    ca_addChild(mZDepthAnimator);
    ca_addChild(mXRotAnimator);
    ca_addChild(mYRotAnimator);
...
SkM44 BasicTransformAnimator::getRelativeTransform3DAtFrame(const qreal relFrame, SkM44* postTransform) const {
    ...
    qreal rotY = mYRotAnimator ? mYRotAnimator->getEffectiveValue(relFrame) : 0.0;

    matrix.preConcat(SkM44::Rotate({0, 0, 1}, qDegreesToRadians(rotZ)));
...
QrealAnimator *BasicTransformAnimator::getZDepthAnimator() const {
    return mZDepthAnimator.get();
}

QrealAnimator *BasicTransformAnimator::getXRotAnimator() const {
    return mXRotAnimator.get();
}

QrealAnimator *BasicTransformAnimator::getYRotAnimator() const {
    return mYRotAnimator.get();
}
...
SkM44 BasicTransformAnimator::getTotalTransform3DAtFrame(const qreal relFrame) const {
    if(mParentTransform) {
        const qreal absFrame = prp_relFrameToAbsFrameF(relFrame);
        const qreal parentRelFrame = mParentTransform->prp_absFrameToRelFrameF(absFrame);
        SkM44 pMatrix = mParentTransform->getTotalTransform3DAtFrame(parentRelFrame);
        pMatrix.preConcat(getRelativeTransform3DAtFrame(relFrame));
        return pMatrix;
    } else {
        return getRelativeTransform3DAtFrame(relFrame);
    }
}
