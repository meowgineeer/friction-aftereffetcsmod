...
void BoundingBox::setTrackMatte(const TrackMatte matte)
{
    if (mTrackMatte == matte) { return; }
    {
        prp_pushUndoRedoName(tr("Set Track Matte"));
        UndoRedo ur;
        const auto oldValue = mTrackMatte;
        const auto newValue = matte;
        ur.fUndo = [this, oldValue]() { setTrackMatte(oldValue); };
        ur.fRedo = [this, newValue]() { setTrackMatte(newValue); };
        prp_addUndoRedo(ur);
    }
    mTrackMatte = matte;
    prp_afterWholeInfluenceRangeChanged();
...
void BoundingBox::setupWithoutRasterEffects(const qreal relFrame,
    ...
    if (mTransformAnimator) {
        data->fRelTransform3D = mTransformAnimator->getRelativeTransform3DAtFrame(relFrame);
        data->fTotalTransform3D = mTransformAnimator->getTotalTransform3DAtFrame(relFrame);

        if (scene) {
            auto camera = scene->getActiveCamera();
            // Don't apply camera to the camera itself!
            if (camera && camera != this) {
                SkM44 camMatrix = camera->getCameraMatrix(relFrame);
                SkM44 invCam;
                if (camMatrix.invert(&invCam)) {
                    data->fTotalTransform3D.setConcat(invCam, data->fTotalTransform3D);
                }
            }
        }
    }
