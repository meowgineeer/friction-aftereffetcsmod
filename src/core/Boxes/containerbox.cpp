...
void ContainerBox::prp_setupTreeViewMenu(PropertyMenu * const menu) {
    ...
    const PropertyMenu::PlainSelectedOp<ContainerBox> addMaskOp =
    [](ContainerBox* const box) {
        auto mask = enve::make_shared<MaskBox>();
    ...
    [](ContainerBox* const box) {
        auto camera = enve::make_shared<CameraBox>();
...
void ContainerBox::drawContained(SkCanvas * const canvas,
    ...
    for(int i = minMax.fMax; i >= minMax.fMin; i--) {
        ...
        if(box->isVisibleAndInVisibleDurationRect()) {
            if (box->getTrackMatte() != TrackMatte::None && i > 0) {
                const auto& matteBox = mContainedBoxes.at(i - 1);
                
                canvas->saveLayer(nullptr, nullptr);
                box->drawPixmapSk(canvas, filter, drawId, delayed);
                if(!box->isGroup()) drawId++;
                
                if (matteBox->isVisibleAndInVisibleDurationRect()) {
                    SkPaint mattePaint;
                    mattePaint.setBlendMode(
                        box->getTrackMatte() == TrackMatte::Alpha ? SkBlendMode::kDstIn :
                        box->getTrackMatte() == TrackMatte::AlphaInverted ? SkBlendMode::kDstOut : SkBlendMode::kDstIn);
                        
                    if (box->getTrackMatte() == TrackMatte::Luma || box->getTrackMatte() == TrackMatte::LumaInverted) {
                        ...
                        mattePaint.setColorFilter(SkColorFilters::Matrix(colorMatrix));
                        if (box->getTrackMatte() == TrackMatte::LumaInverted) {
                            mattePaint.setBlendMode(SkBlendMode::kDstOut);
                        }
                    }
                    
                    canvas->saveLayer(nullptr, &mattePaint);
                    matteBox->drawPixmapSk(canvas, filter, drawId, delayed);
                    if(!matteBox->isGroup()) drawId++;
                    canvas->restore();
                }
                canvas->restore();
                ...
                const auto& nextNextBox = i == 0 ? nullptr : mContainedBoxes.at(i - 1);
                handleDelayed(delayed, drawId, matteBox, nextNextBox);
                continue;
            } else {
                box->drawPixmapSk(canvas, filter, drawId, delayed);
                if(!box->isGroup()) drawId++;
            }
        } else if (box->getTrackMatte() != TrackMatte::None && i > 0) {
            i--;
            handleDelayed(delayed, drawId, box, nextBox);
            const auto& matteBox = mContainedBoxes.at(i);
            const auto& nextNextBox = i == 0 ? nullptr : mContainedBoxes.at(i - 1);
            handleDelayed(delayed, drawId, matteBox, nextNextBox);
            continue;
        }
...
void ContainerBox::processChildrenData(const qreal relFrame,
    ...
    for(int i = minMax.fMax; i >= minMax.fMin; i--) {
        ...
        if (box->getBoxType() == eBoxType::mask) {
            auto maskBox = static_cast<MaskBox*>(box.get());
...
qsptr<BoundingBox> createBoxOfNonCustomType(const eBoxType type) {
    switch(type) {
        ...
        case(eBoxType::mask):
            return enve::make_shared<MaskBox>();
