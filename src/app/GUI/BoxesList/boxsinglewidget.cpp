...
QPixmap* BoxSingleWidget::STOPWATCH_ICON;
QPixmap* BoxSingleWidget::EXPRESSION_ICON;
QPixmap* BoxSingleWidget::CUSTOM_VISIBLE_ICON;
...
void BoxSingleWidget::clearStaticPixmaps()
{
    ...
    delete STOPWATCH_ICON;
    delete EXPRESSION_ICON;
    delete CUSTOM_VISIBLE_ICON;
}

void BoxSingleWidget::mousePressEvent(QMouseEvent *event) {
    if(!mTarget) return;

    if (mStopwatchRect.contains(event->pos())) {
        switchRecordingAction();
        return;
    }

    if (mExpressionRect.contains(event->pos())) {
        if (const auto qrealAnim = enve_cast<QrealAnimator*>(mTarget->getTarget())) {
            DialogsInterface::sGetInstance()->showExpressionDialog(qrealAnim);
            return;
        }
    }
...
void BoxSingleWidget::paintEvent(QPaintEvent *) {
    ...
    int currentX = nameX + textWidth + 10;
    int iconSize = eSizesUI::widget;
    int iconY = (eSizesUI::widget - iconSize) / 2;

    if (enve_cast<Animator*>(prop) && !enve_cast<eBoxOrSound*>(prop)) {
        mStopwatchRect = QRect(currentX, iconY, iconSize, iconSize);
        if (STOPWATCH_ICON && !STOPWATCH_ICON->isNull()) {
            p.drawPixmap(mStopwatchRect, *STOPWATCH_ICON);
        }
        currentX += iconSize + 5;
    } else {
        mStopwatchRect = QRect();
    }

    if (enve_cast<QrealAnimator*>(prop)) {
        mExpressionRect = QRect(currentX, iconY, iconSize, iconSize);
        if (EXPRESSION_ICON && !EXPRESSION_ICON->isNull()) {
            p.drawPixmap(mExpressionRect, *EXPRESSION_ICON);
        }
    } else {
        mExpressionRect = QRect();
    }
